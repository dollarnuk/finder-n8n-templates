...
### BUG-012 · "Loading error" при відкритті воркфлоу (деталей)
- **Дата**: 2026-02-16
- **Симптоми**: При кліку на будь-який воркфлоу у модальному вікні з'являється "Loading error" замість деталей.
- **Причина**: У `database.py` функція `_parse_json_fields()` перетворювала `json_content` з JSON-рядка в dict/об'єкт. Але фронтенд (`index.html`, рядок 2683) викликає `wf.json_content?.substring(0, 1500)` — це метод рядка, який не існує на об'єкті. JavaScript TypeError потрапляв у `catch` блок `openDetail()` і показувався як "Loading error".
- **Рішення**:
  - Прибрано `json_content` зі списку `json_cols` у `_parse_json_fields()` в `database.py`. Тепер `json_content` залишається рядком, як очікує фронтенд.
  - В `app.py` додано захисну перевірку: якщо `json_content` — об'єкт, він серіалізується назад у рядок перед відправкою у `/api/workflow/{id}/json`.
- **Поточна статистика**: 139 воркфлоу, 134 проаналізовано (5 без AI-аналізу).
